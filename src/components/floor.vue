...
<template>
<h3 class="mb-5 text-lg font-medium text-gray-900 dark:text-white ml-2 mr-2 text-center">Kat Seçimi</h3>
<ul class=" flex justify-center gap-2 ml-2 mr-2 animate-pulse p-6">
    <li v-for="(floor,count) in floors" :key="count" @click="selectedfloor(floor.id)" class="relative">
      <div class="md:absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white bg-red-500 border-2 border-white rounded-full -top-2 right-2 dark:border-gray-900">{{floor.floor_using_status }}</div>
        ...
        <label for="react-option" class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">                           
            <div class="block justify-center">
                ...
                <div class="w-full text-lg font-semibold">{{ floor.floor_name }}</div>
                
                <div class="w-full bg-gray-200 rounded-full dark:bg-gray-700">
                    <div class="bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full" :style="'width:'+ statusUsing(floor.floor_capacity,floor.floor_using_status)  +'%'"> {{statusUsing(floor.floor_capacity,floor.floor_using_status)}}%</div>
                </div>
